...
console.log("21. prompt v5 mengikuti karakter dan body plan objek");
{
  ...
  const idle =
    template.split("TOP LEFT — IDLE")[1]?.split("TOP RIGHT — BATTLE")[0] ?? "";
  assert.ok(
...
);
{
  ...
  for (const element of ELEMENT_ROSTER) {
    ...
    for (const target of strengths) {
      assert.ok(
        ...
        `${element}→${target} harus ada di roster`,
      );
  ...
  for (let index = 0; index < ELEMENT_CYCLE.length; index++) {
    ...
    assert.equal(
      ...
      MATCHUP_STRONG,
      `siklus lama ${attacker}→${strongAgainst}`,
    );
  }
...
);
{
  ...
  for (const prompt of [templateV16, faunaV16, evolveV16]) {
    assert.ok(
      prompt.includes("HORIZONTAL FACING LOCK — HOME AND BATTLE CONTRACT"),
    );
...
);
{
  ...
  assert.ok(
    promptV22.includes("round ceramic pot → segmented ceramic torso core"),
  );
