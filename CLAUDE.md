...
- **Tap Collection membuka bottom sheet, bukan langsung mengganti companion.** Sheet memperlihatkan lima base stat dan empat care meter; care disinkronkan server sekali per Anima per kunjungan Collection dan respons basi ditolak dengan revision. `View Profile` membuka row pilihan tanpa mengaktifkannya. `Summon` menyiapkan art dulu, lalu dissolve companion lama, membuka portal, menyimpan `GameState.last_anima`, dan reveal di Home; gagal sebelum art siap tidak boleh mengganti active companion. Tidak ada biaya, model call, atau state aktif baru di database.
...
- **Setelah setiap scan selesai, popup rename muncul dengan nama generated sebagai nilai awal.** Save PATCH `nickname`, Skip tidak menulis jaringan. Nama lama tidak dimigrasi karena database tidak bisa membedakan hasil model dari nickname manual.
...
godot --headless --path game --script res://tests/test_i18n.gd          # 1050 check katalog + key + formatter + wrapping
