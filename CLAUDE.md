...
- Migration Battle `20260813103446_battle_vertical_slice`, indeks bot `20260813105258_index_battle_bot_anima`, cap reward `20260813174007_limit_daily_battle_rewards`, indeks unik ledger `20260813174454_index_battle_reward_ledger_ref`, dan perbaikan status `20260813180241_refine_battle_reward_status` sudah live. `battle_anima` version 3 ACTIVE dengan `verify_jwt=true`; operasi `status` terukur 200 pada user anonim sementara lalu user-nya dihapus. Probe SQL production membuktikan win ketiga dibayar dan win keempat menjadi Training tanpa satu pun mutation progression; harness transport `live_battle.gd` terakhir tertahan sebelum endpoint oleh sesi uji lama (`Invalid Refresh Token: Already Used`), jadi jangan salah membacanya sebagai kegagalan formula atau migrasi.
...
godot --headless --path game --script res://tests/test_scan_ui.gd       # 366 check shell + Battle + komponen + tap + touch + reduced motion
godot --headless --path game --script res://tests/test_i18n.gd          # 1373 check katalog + key + formatter + wrapping
godot --headless --path game --script res://tests/test_game_rules.gd    # 30 check care + kontrak event Battle
