...
- Migration Battle `20260813103446_battle_vertical_slice` dan indeks bot `20260813105258_index_battle_bot_anima` sudah live. `battle_anima` version 1 juga ACTIVE dengan `verify_jwt=true`; smoke production melewati tiga action + replay + forfeit tanpa model call.
...
godot --headless --path game --script res://tests/test_scan_ui.gd       # 249 check shell + Battle + touch + reduced motion
godot --headless --path game --script res://tests/test_i18n.gd          # 1313 check katalog + key + formatter + wrapping
