...
- **Duel/Team/Expedition arena**: dunia Battle full-screen di bawah Chrome dan
  ...
  battle). Zoom `FighterLayer` mengubah seluruh karakter dunia bersama—Anima
  dan kedua Seeker—supaya rasio tinggi kanonis tidak rusak ketika kamera mundur,
  termasuk sepanjang Switch. Refresh authoritative yang tidak mengganti
  petarung mempertahankan framing yang sudah settle, dan background Team bundled
  maupun custom mengikuti parallax Switch 1,0–1,08×; Duel tetap cover 1,0×.
...
- Art **Seeker Roster** juga statis dan ikut ter-bundel ke build, bukan diunduh (ADR-0002): `game/assets/seekers/<slug>.png` dengan empat slug teks (`androgynous` default, `masculine`, `feminine`, `automaton`) plus manifest bersamanya di `SeekerRoster`, memakai kontrak Seeker Sheet yang sama seperti Boss Seeker chapter. Keempat art final sudah dibayar dan ter-commit (prompt `seekers/v2`), terukur **1,93 MiB di PCK** — di bawah taksiran ~3,2 MB ADR-0002. Prompt historis `roster_sheet.md` menggambar umur, tinggi piksel source, dan rangka yang sama; jangan edit prompt yang sudah dibayar itu. Tinggi dunia kanonis sekarang metadata kosmetik `SeekerRoster`: `androgynous` 172 cm, `masculine` 176 cm, `feminine` 170 cm, `automaton` 180 cm; dictionary sheet membawanya ke Duel/Team/Expedition, dan Boss opening menjaga rasio sentimeter player↔Boss secara linear. File `figures/<slug>.md` hanya memiliki geometri outfit, rambut, palet, dan gender read. Menyentuh `roster_sheet.md` mengubah hash prompt keempat slug sekaligus, jadi satu kalimat di sana berharga $0.28, bukan $0.07. Yang berbayar `backend/tools/generate_seeker_art.mjs`, satu slug per proses tanpa retry, raw-nya ter-commit supaya `--reprocess` gratis, `--check`-nya ikut `npm run selftest`, dan `--strip` menyusun kesembilan pose × keempat slug jadi satu PNG untuk audit visual yang tetap wajib karena hash tidak bisa menilai arah hadap. `node eval/seeker_art.mjs` tinggal alat placeholder untuk figur berikutnya dan **melewati slug yang sheet-nya sudah ada**, sebab placeholder lolos semua pemeriksaan roster sehingga menimpa art berbayar gagal senyap. Plafon ~6 figur, dan `test_sprite_slicing` merah di atas itu supaya ADR-0002 dibaca dulu sebelum roster tumbuh. Perintahnya di [`docs/15-commands.md`](docs/15-commands.md).
- **Seeker Avatar dipilih pemain, tidak diturunkan dari Gender/Birth Year (ADR-0001).** Ia hidup di `profiles.seeker_avatar` (nullable, `CHECK` terhadap slug roster, `NULL` = figur default) dan ditulis client **langsung lewat PATCH PostgREST per kolom di bawah RLS** — bukan RPC dan bukan Edge Function, karena ia kosmetik dan tidak menyentuh mata uang. Karena itu jalurnya optimistis seperti Care/Shop: nol masa tunggu, nol `_set_busy()`, dan rollback kalau representasi yang kembali kosong (nol row berubah). **Onboarding satu-satunya pengecualian**: di sana figur ikut sebagai argumen kelima opsional `complete_seeker_profile`, bukan PATCH kedua, supaya pemain baru tidak bisa berakhir dengan nama tersimpan dan figur tidak. `NULL` di argumen itu berarti "biarkan apa adanya", dan slug di luar roster **diabaikan** alih-alih menggagalkan nama — baris picker yang selalu punya pilihan default tidak boleh punya kuasa menahan pemain di luar namanya sendiri; client memperlakukan slug asing dengan cara yang sama lewat `SeekerRoster.normalize()`. Kartunya dimiliki `SeekerAvatarPicker`, satu komponen yang dipakai kedua layar (kolom dan tinggi kartu disetel di scene). Sekarang terlihat di potret Seeker Profile, dipilih saat onboarding, dan berdiri di ketiga arena — Duel, Team Battle, Expedition — sebagai cermin Boss Seeker, tanpa dialog dan tidak pernah bicara; lawan Duel tetap tanpa figur. `SeekerPresenter` memberi Intro/Idle organic breathing + weight shift pada figur manusia dan Boss, mechanical settle pada Automaton, lalu membunuh kedua loop serta memulihkan baseline sebelum command/reaction/result. Satu-satunya tempat figur **pemain lain** digambar adalah sheet detail Atlas, karena itu satu-satunya surface yang memang sudah menyebut nama pemilik: `gallery` atlas_detail memproyeksikan `owner_avatar` persis di sebelah `owner_name` (NULL profil dinormalkan ke slug default di server; Boss Seeker tetap `null` karena art-nya art chapter), kartu grid tidak ikut, dan roster rival Duel/Team/Expedition tetap tanpa kolom profil pemilik — dipagari skenario 39 `npm run selftest`. Top HUD masih teks. Geometri figurnya (frame width, pusat badan opak, edge/companion clamp dua sisi, lantai z, kolom kamera) hidup di `BattleScale`, bukan disalin per arena. Dua fakta komposisi yang gampang ditambal di tempat yang salah: jarak figur↔Anima dibeli dengan **mencadangkan kolom di kotak fit kamera** (`seeker_reserved_column()`), lalu `seeker_pinned_x()` menjaga figur di antara batas tepi viewport dan tepi badan Anima miliknya — edge clamp mencegah clipping di portrait, companion clamp mencegah figur terlepas ke ujung landscape, sedangkan mendorong anchor Anima ke dalam sudah dicoba dan terukur gagal di portrait; dan lantai z figur **ikut tinggi Anima** lewat `player_seeker_z()`, ambang `anima_behind_seeker()` yang sama dengan Boss Seeker, jadi ia melangkah ke depan Anima-nya sendiri begitu Anima itu melewati 60% tingginya alih-alih hilang di belakang.
...
- Background Home/Duel mencampur pasangan PNG siang–malam di shader berdasarkan
  jam lokal absolut: fajar 05:30–06:30, siang penuh sampai 17:30, dan senja
  17:30–18:30, disampel tiap detik supaya resume tidak melompat. Home, Duel,
  dan Team Battle masing-masing punya varian 9:16 dan 16:9 yang dipilih dari
  aspect viewport. Kaki Home tetap pada ground line 68% portrait / 69%
  landscape di **rect art dasar**, bukan pada viewport. Karena pasangan
  portrait day/night menggambar pusat dais pada row berbeda, background
  portrait diberi overscan 1,11× lalu focal point dais-nya diikat ke Stage;
  background yang bergerak halus saat blend, bukan Anima. Home juga memberi
  contact shadow radial yang mengikuti bbox kaki dan visibility pose. Enam
  background statis Duel/Team portrait/landscape day/night memakai source foot
  row dan pivot zoom 91% seperti Expedition/Boss; Duel tetap cover 1,0×,
  sedangkan Team memberi background parallax 1,0–1,08× yang mengikuti arah zoom
  aktual petarung agar Switch terbaca sebagai gerak kamera tanpa kembali ke
  crop 1,18× yang memangkas langit. Kalibrasi production Duel dan Team sama:
  portrait memindahkan background +100 px dan seluruh FighterLayer −117 px;
  landscape +5 px dan −113 px. Overscan vertikal menambah geometry guard kecil
  tanpa mengubah zoom visual supaya offset portrait tetap aman saat impact.
  Background chapter boleh zoom sampai 1,55×. Duel memakai dock 2×2, Team
  menonjolkan Attack/Special di baris dua kolom lalu Guard/Item/Switch/Retreat
  di baris empat kolom, sedangkan Expedition tetap 3+3. Art Home hidup di root
  canvas di belakang `Stage`; peta node The Sugarworks memakai top-view
  sendiri, dan art chapter dari server tetap menang serta tetap center-crop
  horizontal di combat Expedition.
...
Tujuh hal yang gagal **senyap** — tanpa galat yang menunjuk sebabnya — jadi
ketujuhnya tetap di sini alih-alih di rule ber-glob yang belum tentu dimuat
...
- **Izin `VIBRATE` juga MATI secara default dan tidak diminta saat runtime.** `Input.vibrate_handheld()` tetap berjalan tanpa error, Battle Shake tetap terlihat, dan toggle Haptics tetap On, tetapi Android membuang pulse sebelum mencapai vibrator service kalau manifest APK tidak punya izin itu. Preset wajib memuat `permissions/vibrate=true`; APK yang benar memuat tepat `INTERNET`, `VIBRATE`, dan `CAMERA`. `export_presets.cfg` tetap di-gitignore, jadi `aapt2 dump permissions` sesudah setiap build Android adalah satu-satunya pagar yang bertahan.
...
godot --headless --path game --script res://tests/test_scan_ui.gd       # 1644 check shell + Battle + Shop + Bag + komponen + tap + touch + press/release roster sungguhan + UI/SFX hooks + prediksi turn/care/Summon + rollback + cache boot + Trophy Showcase/evolution/Atlas + dialog Evolve gagal + preflight nama + Seeker Avatar arena + LoadingScreen
...
godot --headless --path game --script res://tests/test_battle_sim_parity.gd # 530 check parity simulasi client vs _shared
