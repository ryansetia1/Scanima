...
- **`mobile_theme.tres` adalah satu-satunya sumber visual UI.** Palette-nya deep navy + cyan/violet + gold, dan variasi semantiknya (`PrimaryButton`, `NavButton`, empat Care button, `ModalPanel`, `StatusPanel`) dipakai di production maupun `anima_demo`. Jangan mengembalikan override StyleBox per-node untuk chrome umum; override lokal hanya untuk empat warna meter yang memang membawa makna gameplay.
- **Semua micro-interaction Control dimiliki `UiJuice`.** Ia memasang press squash, release bounce, hover/focus, reveal, modal scale/fade, dan tween meter sekali secara rekursif; dynamic pose button dipasang saat dibuat. Tween lama dibunuh lewat metadata sebelum tween baru menulis properti yang sama. Jangan menambah tween `scale/modulate` button atau modal langsung di `scan_flow.gd`, karena rapid tap akan menumpuk writer dan meninggalkan Control dalam keadaan gepeng.
- **Visual shell tidak punya texture UI.** `ScanimaBackground` menggambar gradient, chamber ring, particle, glow floor, dan grid dari primitive pada 15 fps; `IncubatorEffect` tetap pemilik telur 30 fps. Stage Anima berada di 57% safe band supaya wajah tidak tertutup header, sedangkan node Incubator di-offset -180 px supaya telurnya tetap berada di chamber saat pose row disembunyikan.
...
godot --headless --path game --script res://tests/test_scan_ui.gd       # 79 check layout + theme + care + incubator
