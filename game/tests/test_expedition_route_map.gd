...
func _run() -> void:
	...
	_check(
		(view.get_node("%ExpeditionPartyMeta") as Label).text.contains("A")
		and (view.get_node("%ExpeditionPartyMeta") as Label).text.contains("Battle"),
		"route preview keeps party HP beside node details"
	)
	...
	_check(
		...
		"map displays authoritative daily Expedition Bits"
	)
	_check(
