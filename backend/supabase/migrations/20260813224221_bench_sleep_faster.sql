-- Tidur di Collection mengisi Energy dalam 3 jam (companion tetap 6 jam +5 EXP).
-- Anima di bangku tetap tidak auto-bangun dan tidak kena EXP tidur penuh.
...
begin
  if p_action not in ('sync', 'feed', 'clean', 'sleep', 'wake', 'play', 'summon') then
  ...
  select bits, active_anima_id into v_bits, v_active_id
