-- Starter lifetime 4 Scan, Hunger aktif 4/jam, Collection safe-rest,
-- dan harga Food yang lebih longgar. Ledger starter_team menjamin Core
-- keempat sekali per akun; grant mingguan dan cap bank 3 tidak berubah.

create unique index if not exists quota_ledger_starter_team_owner_unique
  on public.quota_ledger (owner_id)
  where reason = 'starter_team';
...
begin
  ...
  select coalesce(sum(delta), 0)::integer into v_granted
   ...
   where owner_id = p_owner
     ...
     and reason in ('starter_guest', 'starter_legacy', 'starter_google', 'starter_team');
  v_delta := greatest(0, 4 - v_granted);

  if exists (
    select 1
      from public.quota_ledger
     where owner_id = p_owner
       and reason in ('starter_google', 'starter_legacy')
  ) then
    v_reason := 'starter_team';
  else
    v_reason := 'starter_google';
  end if;
  ...
  if v_delta > 0 then
    ...
    values (p_owner, 'genesis_cores', v_delta, v_reason);
  end if;
...
grant execute on function public.upgrade_seeker_account(uuid) to service_role;

-- Akun Google yang sudah linked mendapat Core keempat sekali. Guest tidak disentuh.
do $$
declare
  v_owner uuid;
  v_granted integer;
...
begin
  ...
  loop
    ...
    update public.profiles
     ...
     where id = v_owner;
    insert into public.quota_ledger (owner_id, currency, delta, reason)
    values (v_owner, 'genesis_cores', v_delta, v_reason);
  end loop;
end $$;

update public.catalog_items set price = 1 where id = 'byte_berry';
update public.catalog_items set price = 2 where id = 'moon_biscuit';
update public.catalog_items set price = 2 where id = 'moss_wrap';
update public.catalog_items set price = 3 where id = 'spark_skewer';
update public.catalog_items set price = 4 where id = 'prism_jelly';
update public.catalog_items set price = 5 where id = 'ember_noodles';
update public.catalog_items set price = 6 where id = 'cloud_curry';
update public.catalog_items set price = 8 where id = 'star_bento';
update public.catalog_items set price = 10 where id = 'nova_feast';

drop function if exists public.apply_care(uuid, uuid, text, text, text);
