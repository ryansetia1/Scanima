-- Expedition encounter lifecycle. The Edge resolver computes combat state;
-- these service-only RPCs lock rows and commit items, EXP, Supplies, rollback,
-- first-clear Bits, and Trophy ownership atomically.
...
begin
  ...
  select array_agg((member->>'anima_id')::uuid order by (member->>'anima_id')::uuid)
    into v_opponent_ids from jsonb_array_elements(p_opponent_snapshot) member;
  select array_agg((member->>'anima_id')::uuid order by (member->>'anima_id')::uuid)
    into v_state_ids
    from jsonb_array_elements(p_initial_state #> '{opponent,roster}') member;
  if v_state_ids is distinct from v_opponent_ids then raise exception 'SNAPSHOT_MISMATCH'; end if;
  if exists (
...
create or replace function public.reset_expedition_zone(
  p_run public.expedition_runs,
  p_retry_map jsonb,
  p_retry_seed text
) returns public.expedition_runs
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_run public.expedition_runs;
  v_checkpoint jsonb := p_run.checkpoint_state;
begin
  if jsonb_typeof(p_retry_map) <> 'object'
     or jsonb_typeof(p_retry_map->'entry') <> 'array'
     or jsonb_typeof(p_retry_map->'nodes') <> 'array'
     or p_retry_seed is null or length(p_retry_seed) not between 1 and 128 then
    raise exception 'INVALID_EXPEDITION_MAP';
  end if;
  perform 1 from public.profiles where id = p_run.owner_id for update;
  if p_run.bits_refresh_spent > 0 and p_run.shop_refresh_action_id is not null
     and not exists (
       select 1 from public.quota_ledger
        where reason = 'expedition_refund' and ref_id = p_run.shop_refresh_action_id
     ) then
    update public.profiles set bits = bits + p_run.bits_refresh_spent
     where id = p_run.owner_id;
    insert into public.quota_ledger (owner_id, currency, delta, reason, ref_id)
    values (
      p_run.owner_id, 'bits', p_run.bits_refresh_spent,
      'expedition_refund', p_run.shop_refresh_action_id
    );
  end if;
  update public.expedition_runs set
    status = 'active',
    ...
    version = version + 1,
    seed = p_retry_seed,
    zone_map = p_retry_map,
    available_node_ids = p_retry_map->'entry',
    current_node_id = null,
    nodes_completed = 0,
    supplies = coalesce((v_checkpoint->>'supplies')::integer, 0),
    boosts = coalesce(v_checkpoint->'boosts', '[]'::jsonb),
    party_state = coalesce(v_checkpoint->'party_state', '[]'::jsonb),
    pending_node = null,
...
create or replace function public.expedition_party_after_encounter(
  p_snapshot jsonb,
  p_state jsonb
) returns jsonb
language sql
...
as $$
  select coalesce(jsonb_agg(
    snapshot.member || jsonb_build_object(
      'hp', (fighter.member->>'hp')::integer,
      'current_hp', (fighter.member->>'hp')::integer,
      'max_hp', (fighter.member->>'max_hp')::integer
    )
    order by snapshot.ordinality
  ), '[]'::jsonb)
  from jsonb_array_elements(p_snapshot)
       with ordinality snapshot(member, ordinality)
  join jsonb_array_elements(p_state #> '{player,roster}') fighter(member)
    on fighter.member->>'anima_id' = snapshot.member->>'anima_id'
$$;
...
begin
  ...
  select array_agg((member->>'anima_id')::uuid order by (member->>'anima_id')::uuid)
    into v_state_ids from jsonb_array_elements(p_state #> '{opponent,roster}') member;
  if v_state_ids is distinct from v_snapshot_ids then raise exception 'SNAPSHOT_MISMATCH'; end if;
...
create or replace function public.forfeit_expedition_encounter(
  p_owner uuid,
  p_encounter_id uuid,
  p_retry_map jsonb,
  p_retry_seed text
...
begin
  ...
  if v_encounter.status = 'active' then
    update public.expedition_encounters set
      status = 'forfeited', finished_at = now(), updated_at = now()
    where id = v_encounter.id returning * into v_encounter;
    v_run := public.reset_expedition_zone(v_run, p_retry_map, p_retry_seed);
  end if;
  return jsonb_build_object(
...
create trigger expedition_encounters_count_seeker_victory
after update of status on public.expedition_encounters
for each row
...
revoke all on function public.start_expedition_encounter(
  uuid, uuid, integer, jsonb, jsonb, jsonb, jsonb, text, integer, text
) from public, anon, authenticated;
revoke all on function public.reset_expedition_zone(
  public.expedition_runs, jsonb, text
) from public, anon, authenticated;
revoke all on function public.expedition_party_after_encounter(jsonb, jsonb)
  from public, anon, authenticated;
...
revoke all on function public.forfeit_expedition_encounter(
  uuid, uuid, jsonb, text
) from public, anon, authenticated;

grant execute on function public.expedition_encounter_payload(public.expedition_encounters)
  to service_role;
grant execute on function public.start_expedition_encounter(
  uuid, uuid, integer, jsonb, jsonb, jsonb, jsonb, text, integer, text
) to service_role;
grant execute on function public.reset_expedition_zone(
  public.expedition_runs, jsonb, text
) to service_role;
grant execute on function public.expedition_party_after_encounter(jsonb, jsonb)
  to service_role;
grant execute on function public.commit_expedition_turn(
  uuid, uuid, integer, integer, text, text, smallint, text,
  jsonb, jsonb, jsonb, jsonb, boolean, boolean, jsonb, text
) to service_role;
grant execute on function public.forfeit_expedition_encounter(
  uuid, uuid, jsonb, text
) to service_role;
