-- Battle dan Training memakai entry gate yang sama. Energy disinkronkan lebih
-- dulu supaya jam device tidak bisa mempertahankan Anima lelah sebagai eligible.
...
begin
  ...
  if coalesce((v_player.care->>'energy')::numeric, 0) < 20 then
...
-- CREATE OR REPLACE mempertahankan ACL, tetapi grant eksplisit menjaga invariant
-- fungsi uang/progression tetap hanya dapat dipanggil Edge Function service role.
