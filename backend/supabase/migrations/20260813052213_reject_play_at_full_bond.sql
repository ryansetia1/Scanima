-- Play exists to build Bond. At 100 it previously spent 5 Energy and could
-- still grant invisible care_score, which made the disabled client state
-- bypassable through a direct Edge Function request. Reassert the rule in the
-- transaction that owns care and currency.
