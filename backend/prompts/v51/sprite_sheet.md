Create a polished 2D character sheet for ONE original creature-monster derived
...
Keep the same camera: forward-left three-quarter view from near eye-level.
Full body visible in every character cell at comparable scale.

GROUND PLANE LOCK — GROUNDED POSES
For every creature whose body plan moves or rests on land, establish one flat
implied ground plane parallel to the bottom edge. Idle, Happy, Hungry, Dirty,
and Damaged are grounded poses. Sleep rests on that same plane. Battle may
leave it only when the action is clearly a jump or intentional flight.

When a grounded body has two or more distinct weight-bearing supports, at least
two clearly separated support endpoints must touch the same narrow lowest band.
If its anatomy has support in both the front half and rear half, one front half
support and one rear half support must both touch that band. Never make one
foreground paw, wheel, foot, root, or limb the sole lowest contact while the
rear support hangs visibly above it. A standing biped plants both feet. A broad
base, rooted body, or serpentine body contacts the plane through the continuous
load-bearing area appropriate to its anatomy. A single-legged body plants its
sole support. Only an explicitly hovering or flying body plan may remain clear
of the plane.

This is a perspective and silhouette contract, not permission to draw a floor
or shadow. Keep natural overlap and depth, but do not tilt the implied ground
plane steeply into the canvas.
...
TOP LEFT — IDLE
For every land body, this is a grounded pose: apply the Ground Plane Lock before expression or acting.
NEUTRAL IDLE GAZE — IDLE CELL ONLY
Keep the head, face plane, or equivalent leading sensory plane relaxed and level along its natural forward axis; do not crane, pitch, or tilt it up or down.
This Idle-only lock overrides any character direction, personality, authority pose, reference gaze, or expression that asks the head or eyes to lift, lower, look away from the head axis, or meet the viewer.
When gaze direction is naturally readable, focus every pupil or iris on one distant target straight ahead along the head or snout direction. Keep the gaze anatomically relaxed: never look at the viewer or camera, look up or down, or roll the eyes sideways into a side-eye.
IDLE THREE-QUARTER GEOMETRY LOCK — IDLE CELL ONLY
Stage the resting head and body in a conventional forward-left three-quarter view, about 30–45 degrees away from front view, never a 90-degree side profile.
Keep both the front plane and one side plane of the face, muzzle, nose, or leading sensory structure readable. For naturally paired eyes, keep both near and far eyes visible when anatomy and normal occlusion allow, with natural foreshortening.
Freeze this head pose before setting the gaze. Aim only the pupils or irises within their natural range along the already-established head or snout axis; never yaw, turn, pitch, or tilt the head to aim the eyes.
If exact eye targeting would conflict with that resting presentation, preserve the three-quarter geometry and use relaxed pupils centered within their sockets; never hide the far eye or flatten the face into profile.
For pupil-less, solid-glow, compound, single, hidden, or faceless sensory anatomy, preserve that anatomy. Do not add pupils, irises, eyes, or a new face to manufacture a readable gaze.
...
TOP CENTER — BATTLE
If this action stays on land, apply the Ground Plane Lock. Leave the plane only for a clearly readable jump or intentional flight.
...
TOP RIGHT — SLEEP
For every land body, its lowered or curled body visibly rests on the same implied ground plane; never suspend it above that plane.
