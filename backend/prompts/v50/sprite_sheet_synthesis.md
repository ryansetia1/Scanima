Create a polished 2D character sheet for ONE original Scanima Result Anima,
...
Use the same forward-left three-quarter camera from near eye-level. Full body
visible at comparable scale.

GROUND PLANE LOCK — GROUNDED POSES
For every creature whose body plan moves or rests on land, establish one
shallow implied perspective ground plane. Idle, Happy, Hungry, Dirty, and
Damaged are grounded poses. Sleep rests on that same plane. Battle may leave it
only when the action is clearly a jump or intentional flight.

SOLID SUPPORT CONTACT — SYNTHESIS OVERRIDE
Every visible weight-bearing support in every grounded pose must terminate in
actual solid body pixels at its own contact point: paw pad, foot sole, wheel
tread, track, root base, or the load-bearing underside appropriate to that
anatomy. Give each contact a short flattened tangent or subtle weight
compression so it visibly bears mass without changing the body plan. A broad,
rooted, or serpentine body uses its natural continuous load-bearing area; a
single-legged body plants its sole support. Only an explicitly hovering or
flying body plan may remain clear of the plane.

Preserve natural forward-left three-quarter overlap and near/far depth. The
projected vertical offset from a nearer contact to a farther contact must not
exceed half of that support's own height or radius. Never let a rear support
hang high enough to read as dangling or airborne.

The support anatomy itself must provide contact. Glow, aura, sparks, dust,
debris, speed trails, puddles, floor marks, and shadows do not count as contact
and must not bridge a gap beneath a support. Do not add a visible floor or
ground plane. This is a body-silhouette and perspective contract.
 Treat every cell as an invisible sealed safe box. Keep
