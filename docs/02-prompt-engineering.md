...
2. `extractJson()` di `backend/supabase/functions/_shared/vision.mjs` menangani bentuk keluaran yang wajar tapi tidak diminta: bungkus ```json (termasuk pagar yang tidak ditutup), kalimat pengantar sebelum objek, array potongan string yang harus disambung, dan JSON yang terpotong di tengah string/kurung. Ia menutup struktur yang sudah tertulis; ia tidak mengarang field yang tidak sempat keluar.
