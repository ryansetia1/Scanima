...
> **Status per 13 Agustus 2026.** Pipeline sudah terbukti dengan foto sungguhan pada nano-banana-pro dan GPT Image 2. Setelah A/B medium vs high, default dipindah ke GPT Image 2 medium dan prompt production v2 mengikuti anime cel-shaded style guide. Smoke Set berbayar dengan template v2 final sudah menjalankan 3/3 sheet 4/4 sel dan gate 2/2. V3 menghilangkan logo Nike lewat marking pengganti dan tetap menjadi default production, tetapi marking ciptaan itu kemudian terbukti bisa terlihat seperti logo pada objek polos, sementara contoh DAMAGED universal membuat bahan non-elektronik tampak cyborg. Candidate **v4** menghapus emblem pengganti dan membawa `surface_finish` + `damage_hints` material-spesifik dari Vision; 20 skenario gratis dan dry-run lima foto lulus, tetapi v4 belum dipromosikan sebelum Smoke Set visual berbayar disetujui. Sisi Godot kini punya 159 pemeriksaan headless: client state 34, slicing/presenter 75, dan kontrak UI/inkubator 50.
...
**Koleksi dasar, Stats, sizing mobile, dan inkubator sekarang selesai di Phase 2.** Client membaca ulang row Anima `ready` dari Postgres di bawah RLS, bukan menyimpan roster lokal kedua. Modal `ItemList` dua kolom memakai thumbnail idle 128px hanya jika sheet sudah ada di cache; art yang belum lokal memakai placeholder dan baru mengunduh sheet saat dipilih. Stats memakai row server yang sama untuk HP/ATK/DEF/SPD/SPECIAL, element, rarity, dan stage. Basis 720×1280 memakai font body/button 32px, heading 44–48px, target sentuh 96px (sekitar 48dp), dan spacing 16px. Stage tidak lagi fixed; ia mengikuti viewport. Safe area Android/iOS dikonversi dari piksel fisik ke koordinat viewport, wajib untuk target SDK 35 yang edge-to-edge. Selama generation, `IncubatorEffect` mengganti foto/Anima lama dengan telur energi procedural; sukses memicu burst + hatch reveal, sedangkan gagal/timeout mengembalikan Anima lama. `test_scan_ui.gd` menjaga 50 kontrak anchor, kontrol, theme, ukuran portrait, dan lifecycle inkubator tanpa jaringan.
...
Dengan itu, tujuh dari exit criteria fase ini sudah terbukti. Yang belum: loop perawatan, dan satu pemeriksaan yang tidak bisa dijalankan headless — kamera harus dicoba di perangkat sungguhan, dengan foto benda yang spesiesnya sudah ada di `species_library` supaya jalurnya cache hit (~$0.003, bukan $0.07). APK-nya sekarang ada, jadi yang tersisa cuma perangkatnya.
