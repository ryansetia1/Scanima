...
**Koleksi dasar, Stats, dan sizing mobile sekarang selesai di Phase 2.** Client membaca ulang row Anima `ready` dari Postgres di bawah RLS, bukan menyimpan roster lokal kedua. Modal `ItemList` dua kolom memakai thumbnail idle 128px hanya jika sheet sudah ada di cache; art yang belum lokal memakai placeholder dan baru mengunduh sheet saat dipilih. Stats memakai row server yang sama untuk HP/ATK/DEF/SPD/SPECIAL, element, rarity, dan stage. Basis 720×1280 memakai font body/button 32px, heading 44–48px, target sentuh 96px (sekitar 48dp), dan spacing 16px. Stage tidak lagi fixed; ia mengikuti viewport. Safe area Android/iOS dikonversi dari piksel fisik ke koordinat viewport, wajib untuk target SDK 35 yang edge-to-edge. `test_scan_ui.gd` menjaga 42 kontrak anchor, kontrol, theme, dan empat ukuran portrait tanpa jaringan.
